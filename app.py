...
if st.button("Run Risk Analysis"):
    if model and scaler:
        ...
        try:
            ...
            # Use the specific index for 'Serious' (usually Index 1 if Yes=1)
            proba_serious = model.predict_proba(scaled_data)[0][1]
            
            st.divider()
            # Verify if prediction is 1 (Serious) OR if probability is > 0.5
            if prediction == 1:
                st.error(f"### ⚠️ HIGH RISK: SERIOUS OUTCOME PREDICTED")
                st.write(f"Confidence Level: **{proba_serious:.2%}**")
            else:
                st.success(f"### ✅ LOW RISK: NOT SERIOUS")
                # Showing the probability of seriousness even if it is low
                st.write(f"Probability of Seriousness: **{proba_serious:.2%}**")
        except Exception as e:
